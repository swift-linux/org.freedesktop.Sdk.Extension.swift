...
install) run flatpak --user install swift-local `basename "$file" .json`; exit ;;
uninstall) run flatpak --user uninstall `basename "$file" .json`; exit ;;
update) run flatpak --user update `basename "$file" .json`; exit ;;
*) echo 'Invalid argument.'; exit 1 ;;
esac
